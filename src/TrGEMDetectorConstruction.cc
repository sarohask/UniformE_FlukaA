...
G4VPhysicalVolume* TrGEMDetectorConstruction::Construct() 
   ...
   //rotaion matrix for staright pipes
  	...
  	worldLog->SetVisAttributes(worldAttributes) ;
		...
		newBox = new G4Box("NewBox", worldSizeX, worldSizeY, 0.1*mm) ;
		newLog = new G4LogicalVolume( newBox, fAirMat, "NewLog") ;
		new G4PVPlacement ( 0,
									  		G4ThreeVector(0., 0., -39.55*mm),
									  		newLog ,
									  		"NewFakePhy",
									  		worldLog,
									 		 	false,
  	...
  	// Detector Thickness
  	detectorThickness = accumulate(thickness , thickness+30, detectorThickness);
		singleChamberThickness = (detectorThickness + 19.984)*mm;
  	gapInChambers = 3.7*mm;
  	
  	// Chamber values
  	double chamberZPos[numChamber] = { -(singleChamberThickness + gapInChambers/2.)*mm, (gapInChambers/2.)*mm };
  	cout<<"...................................................................."<<chamberZPos[0]<<endl;
  	// Fake values
  	double fakeThickness = 0.1*mm ;
  	double fakeZPos[numChamber][numFake] = {{(chamberZPos[0] - fakeThickness/2.)*mm, -(chamberZPos[1] - fakeThickness/2.)*mm},
  																				{(chamberZPos[1] - fakeThickness/2.)*mm, -(chamberZPos[0] - fakeThickness/2.)*mm}} ;
  																					
  	//double fakeZPos[numChamber][numFake] = {{(chamberZPos[0] - fakeThickness/2.)*mm, -(chamberZPos[1] - fakeThickness/2.)*mm},
  																					//{-(chamberZPos[0] - fakeThickness/2.)*mm, (chamberZPos[1] - fakeThickness/2.)*mm}} ;
  																					
  	cout<<fakeZPos[0][0]<<"...."<<fakeZPos[0][1]<<"...."<<fakeZPos[1][0]<<"...."<<fakeZPos[1][1]<<endl;
  	// Lateral fake values
  	double lateralFakeThick = (singleChamberThickness + 2*fakeThickness)*mm ;
		double lateralFakeZPos[numChamber] = {-(singleChamberThickness/2. + gapInChambers/2)*mm, (singleChamberThickness/2. + gapInChambers/2.)*mm} ;
  	
  	// Spacers values			
  	double spacerThickness[numSpacer] = {thickness[3], thickness[7], thickness[11], thickness[15]} ;
  	...
  	G4double pullOutX = 19.*mm;
		...
		G4double pullOutZ = (detectorThickness -7.466)*mm ;
  	...
  	double pulloutZPosition[numChamber] = {(chamberZPos[0] + 3.235 + pullOutZ/2.)*mm, (chamberZPos[1] + 3.235 + pullOutZ/2.)*mm} ;									//symmetric
  	...
  	double vFatZPos[numChamber] = {(chamberZPos[0] + detectorThickness + 6.)*mm, (chamberZPos[1] + detectorThickness + 6.)*mm} ;	//symmetric		
  	
  	// OptoHybrid values
  	G4double optoHybridThickness = 1.6*mm ;
	 	G4double optoHybridWidth = 220.13*mm ;
	 	G4double optoHybridHeight = 140.60*mm ;
  	double optoHybridZPos[numChamber] = {(chamberZPos[0] + detectorThickness + 14.5)*mm, (chamberZPos[1] + detectorThickness + 14.5)*mm } ; // Adding 38.4 mm in first position
  	...
  	double coolPadsZPos[numChamber] = {(chamberZPos[0] + detectorThickness + 7.0)*mm, (chamberZPos[1] + detectorThickness + 7.0)*mm } ;      // Adding 38.4 mm in first position
  	...
  	double optoCoolPadsZPos[numChamber] = { (chamberZPos[0] + detectorThickness + 16.0)*mm, (chamberZPos[1] + detectorThickness + 16.0)*mm } ;
  	...
  	double coolPipeZPos[numChamber] = { (chamberZPos[0] + detectorThickness + 11.5)*mm, (chamberZPos[1] + detectorThickness + 11.5)*mm } ;
 		
 		// External frame values
  	double frameThickness = (detectorThickness -7.466)*mm ;
  	double frameZPos[numChamber] = { (chamberZPos[0] + 3.235 + frameThickness/2.)*mm, (chamberZPos[1] + 3.235 + frameThickness/2.)*mm } ;                //symmetric
  	
  	// External zig values
  	G4double zigThickness = ( detectorThickness - 3.235)*mm ;
  	double zigZPos[numChamber] = { (chamberZPos[0] + 3.235 + zigThickness/2.)*mm, (chamberZPos[1] + 3.235 + zigThickness/2.)*mm } ;
  	
  	// Kapton cover values
  	G4double kaptonCoverThickness = 0.050*mm ;
  	double attachedKaptonCoverZPos[numChamber] = { (chamberZPos[0] + detectorThickness + kaptonCoverThickness/2.)*mm, (chamberZPos[1] + detectorThickness + kaptonCoverThickness/2.)*mm } ;
  	double bulgeKaptonCoverZPos[numChamber] = { (chamberZPos[0] + detectorThickness + 18.959)*mm, (chamberZPos[1] + detectorThickness + 18.959)*mm } ;
  	double sideKaptonCoverXPos[2] = {137.778211389*mm, -137.778211389*mm} ;
  	
  	//Aluminium cover values
  	G4double coverThickness = 1.*mm ;
  	double attachedCoverZPos[numChamber] = { (chamberZPos[0] + detectorThickness + kaptonCoverThickness + coverThickness/2.)*mm, (chamberZPos[1] + detectorThickness + kaptonCoverThickness + coverThickness/2.)*mm } ;
  	double bulgeCoverZPos[numChamber] = { (chamberZPos[0] + detectorThickness + 19.484)*mm, (chamberZPos[1] + detectorThickness + 19.484)*mm} ;
  		...
  		{				
    		...
    		G4Trd* outFrame = new G4Trd("OutFrame", (frameThickness)/2.0, (frameThickness)/2.0, (readoutThinBase)/2, (readoutLargeBase)/2, (readoutHeight)/2.);
  			...
  			//vfats starts
   			...
   			std::ifstream coordFile ("VFatCoord.txt", std::ios::in) ;
   			if(coordFile) 
   				{
      			while(!coordFile.eof()) 
      				{
   			...
   			// Placement of VFats -- Here lies the positionning of every single VFat2 module
				...
				for(G4int chamber = 0; chamber < 2 ; chamber++)
  		{	
...
void TrGEMDetectorConstruction::ConstructSDandField()
	{
  	...
  	newLog->SetSensitiveDetector(sensitive) ;
  	for(G4int chamber = 0; chamber < buildChamber ; chamber++)
  		{
